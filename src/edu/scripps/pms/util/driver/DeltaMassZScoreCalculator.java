/**
 * @file DeltaMassZScoreCalculator.java
 * This is the source file for edu.scripps.pms.util.spectrum.DeltaMassZScoreCalculator
 * @author Tao Xu
...
public class DeltaMassZScoreCalculator {
    public static final String USAGE = "\n\n!!! USAGE: deltaMassZScoreCalculator -i input -o output -p pValueCutOff !!!";
...
//System.out.println( seq );
        return checkNTerm(seq) && checkCTerm(seq); 

    }    
    private static boolean checkNTerm(String seq) {
        boolean result = true;
...
//System.out.println(result + "\t" + seq + "\t" + massShift + "\tntermMassShift: " + ntermResidueMass + "\tlowLimit: " + lowLimit + "\thighlimit: " + highLimit);
    ...
    private static boolean checkCTerm(String seq) {
        boolean result = true;
        ...
        for(Protein p : dtaselectProteins) {
            ...
            for(Iterator<Peptide> pepItr=p.getPeptides(); pepItr.hasNext(); ) {
                ...
                if(isModifiedPeptide(peptide) && pValue >= minPValue && peptide.getDeltCnValue() > 0.01) {
                    if(isAcceptable(peptide.getSequence())) {
