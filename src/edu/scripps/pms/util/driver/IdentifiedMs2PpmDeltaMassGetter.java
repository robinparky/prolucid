/**
 * @file IdentifiedMs2PpmDeltaMassGetter.java
 * This is the source file for edu.scripps.pms.util.driver.IdentifiedMs2PpmDeltaMassGetter
 * @author Tao Xu
 * @date $Date
 */
...
public class IdentifiedMs2PpmDeltaMassGetter {
...
//    private static HashSet<String> peptidesInCommon = new HashSet<String>();
    ...
    public static void output(ArrayList<Protein> dtaselectProteins) throws IOException {
        ...
        ps.println("CalculatedMZ\tObservedMz\tChargeAdjustedDeltaMassInPpm\tAbsDeltaMass\tChargeState\t");
        ...
        for(Iterator<Peptide> it = peptides.iterator(); it.hasNext();) {
            ...
            double mplush = Double.parseDouble(peptide.getMhPlus());
            double calcmplush = Double.parseDouble(peptide.getCalcMHplus());
            int chargestate = Integer.parseInt(peptide.getChargeState());
            double calcMz = (calcmplush + chargestate*massh -massh)/chargestate;
            double observedMz = (mplush + chargestate*massh -massh)/chargestate;
            double absdeltamass = calcmplush*deltamass/chargestate/1000000;
            double chargeaddjectdeltamassinppm = deltamass/chargestate;
            ...
            sb.append("\t" + calcMz + "\t" + observedMz + "\t" + chargeaddjectdeltamassinppm + "\t" +  absdeltamass + "\t" + chargestate); 
