...
public class MassRangeFilteringIndex implements DBIndexStore {
    ...
    private float[] minMasses;
    private float[] maxMasses;
    ...
    void init(List<MassRange> massRanges) {
        ...
        minMasses = new float[numRanges];
        maxMasses = new float[numRanges];

        int rangeNum = 0;
        for (MassRange range : massRanges) {
            final float precMass = range.getPrecMass();
            final float tol = range.getTolerance();

            float minMass = precMass - tol;
            if (minMass == 0) {
                minMass = 0f;
            }
            float maxMass = precMass + tol;
    ...
    public void addSequence(float precMass, int sequenceOffset, int sequenceLen, String sequence, String resLeft, String resRight, long proteinId) throws DBIndexStoreException {
        ...
        if (indexedSequence == null) {
            indexedSequence = new IndexedSequence(precMass, sequenceOffset, sequenceLen,
                    sequence, resLeft, resRight, proteinId);
    ...
    public long addProteinDef(long num, String accession, String protSequence) throws DBIndexStoreException {
        //nothing to do, seq-protein associations are stored in indexed sequence itself
        return num;
    }
